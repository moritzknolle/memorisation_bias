"""Training with DP-SGD on MIMIC-IV-ECG.

Same modes and outputs as mimic-ecg.py, with DP-SGD (jax_privacy) applied in train_and_eval.

* --eps sets the privacy budget; delta is set to 1/len(train_historical).
* --one_record_per_patient keeps one ECG per patient in the training split (patient-level DP).
* --eps=inf is the non-private baseline: the same DP training step with the noise multiplier
  set to 0 (see src/training/dp.py).

Usage:
    python mimic-ecg_dp.py --train_random_subset=True --eps=100 --logdir=<log_root>/dp/eps100
    python mimic-ecg_dp.py --train_random_subset=True --eps=100 --one_record_per_patient=True \\
        --logdir=<log_root>/dp/eps100_p
    python mimic-ecg_dp.py --train_random_subset=True --eps=inf --logdir=<log_root>/nonprivate
"""
...
flags.DEFINE_integer("batch_size", 4096, "Batch size. Must be divisible by --microbatch_size.")
flags.DEFINE_float(
    "eps",
    100.0,
...
flags.DEFINE_float("C", 3.0, "Clipping threshold for the per-example L2 gradient norm.")
...
flags.DEFINE_string("wandb_project", "mimic-ecg_dp", "Weights & Biases project to log to.")
flags.DEFINE_bool(
    "use_dp",
    True,
...
def main(argv):
    ...
    (
        ...
        dataset_name="mimic-ecg_p" if FLAGS.one_record_per_patient else "mimic-ecg",
    ...
    # number of optimizer steps the run will actually perform. Has to match the number of steps the
    # DP accountant calibrates the noise for, otherwise the LR schedule is stretched/compressed
    # relative to the privacy budget.
